...
type clientAPI interface {
	...
	Deactivate(ctx context.Context, updater *user.User) error
}
...
func CreateServeMux(api clientAPI, promFactory *promauto.Factory) serveMux {
	...
	router.HandleFunc("/deactivate", server.handleGetPost(server.methodNotAllowedHandler, server.deactivate))
	return router
}
