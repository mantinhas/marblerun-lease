...
func (c *Core) Activate(ctx context.Context, req *rpc.ActivationReq) (res *rpc.ActivationResp, err error) {
	...
	manifest, err := txdata.GetManifest()
	if err != nil {
		c.log.Error("Loading manifest failed", zap.Error(err))
	...
	if p, ok := peer.FromContext(ctx); ok {
		if addr, ok := p.Addr.(*net.TCPAddr); ok {
			clientIP = strings.Split(addr.IP.String(), ":")[0]
		}
	}

	// add marbleip to store
	if err := txdata.PutMarbleIP(req.MarbleType, clientIP); err != nil {
		c.log.Error("Could not add marble IP to store", zap.Error(err))
		return nil, status.Errorf(codes.Internal, "adding marble IP to store: %s", err)
	}
...
// Ping implements the MarbleAPI function to process marble ping request (implements the MarbleServer interface).
//
// Returns a response if the authentication was successful.
// Returns an error if the authentication failed.
func (c *Core) Ping(ctx context.Context, req *rpc.PingReq) (res *rpc.PingResp, err error) {
	c.log.Info("Received ping request") //, zap.String("MarbleType", req.MarbleType))
	...
	defer func() {
		if err != nil {
			c.log.Error("Marble Ping response failed") //, zap.String("marbleType", req.GetMarbleType()), zap.String("uuid", req.GetUUID()))
		}
	}()
	...
	//c.metrics.marbleAPI.activationSuccess.WithLabelValues(req.GetMarbleType(), req.GetUUID()).Inc()
	c.log.Info("Successfully responded to Marble ping", zap.String("MarbleType", req.GetMarbleType()), zap.String("UUID", req.GetUUID()))
