...
func (a *ClientAPI) SetManifest(ctx context.Context, rawManifest []byte) (recoverySecretMap map[string][]byte, err error) {
	...
	rootCertString, quote, err := a.core.GetCertQuote(txdata)
	if err != nil {
		return nil, err
	}
...
// GetSecrets allows a user to retrieve secrets from the Coordinator.
func (a *ClientAPI) Deactivate(ctx context.Context, client *user.User) error {
	a.log.Info("Deactivate called", zap.String("user", client.Name()))

	// verify user is allowed to read the requested secrets
	if !client.IsGranted(user.NewPermission(user.PermissionDeactivate, nil)) {
		a.log.Warn(
			"Deactivate failed: user is not allowed to deactivate",
			zap.String("user", client.Name()),
		)
		return fmt.Errorf("user %s is not allowed to deactivate", client.Name())
	}

	// deactivate marbles
	if err := a.core.DeactivateMarbles(ctx); err != nil {
		a.log.Error("Deactivate failed", zap.Error(err))
	}

	go func() {
		time.Sleep(5 * time.Second)
		a.log.Info("Coordinator exiting")
		os.Exit(1)
	}()
