...
type Manifest struct {
	...
	// Deactivation and ping settings
	DeactivationSettings map[string]Deactivation
}
...
func (m Manifest) Check(zaplogger *zap.Logger) error {
	...
	for roleName, role := range m.Roles {
		...
		case "Deactivate":
			if len(role.ResourceNames) != 0 {
				return fmt.Errorf("role %s: resource names are not allowed for type Deactivate", roleName)
			}
			for _, action := range role.Actions {
				if !(strings.ToLower(action) == user.PermissionDeactivate) {
					return fmt.Errorf("unknown action: %s for type Deactivate in role: %s", action, roleName)
				}
			}
		default:
	...
	for deactivationName, d := range m.DeactivationSettings {
		if deactivationName == "Coordinator" || deactivationName == "Marbles" {
			if deactivationName == "Coordinator" {
				// if ConnectionUrl is not set, error
				if len(d.ConnectionUrl) <= 0 {
					return fmt.Errorf("deactivation settings for Coordinator are missing ConnectionUrl")
				}

				if len(d.ConnectionCertificate) <= 0 {
...
func (m Manifest) IsUpdateManifest() bool {
	return len(m.Infrastructures) == 0 && len(m.Marbles) == 0 &&
		...
		len(m.TLS) == 0 && len(m.DeactivationSettings) == 0
}
...
// Deactivation contains the timeout settings for a deactivation and the url for the coordinator to ping.
type Deactivation struct {
	// ConnectionUrl is the url the coordinator should ping. Only used for the coordinator.
