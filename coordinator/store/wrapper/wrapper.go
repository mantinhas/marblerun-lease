...
// PutMarble ip for use in deactivation
func (s Wrapper) PutMarbleIP(marbleName string, ip string) error {
	return s.put(request.MarbleIP, marbleName, ip)
}

// GetMarbleIP returns the ip of a specific Marble from store.
func (s Wrapper) GetMarbleIP(marbleName string) (string, error) {
	var ip string
	err := s.get(request.MarbleIP, marbleName, &ip)
	return ip, err
}
